    ...
    where TEntity : class, IBaseEntity<TIdentifier>  where TIdentifier : IEquatable<TIdentifier>
{
    ...
    /// <summary>
    /// Get all values from the repository.
    /// </summary>
    ...
    /// <summary>
    /// Get value from the repository the value where the id matches.
    /// </summary>
    /// <param name="id">The id to match.</param>
    /// <returns>The value that matches the id.</returns>>
    public async Task<TEntity?> GetByIdAsync(TIdentifier id)
    {
        return await context.Set<TEntity>().FirstOrDefaultAsync(n => n.Id.Equals(id));
    }
